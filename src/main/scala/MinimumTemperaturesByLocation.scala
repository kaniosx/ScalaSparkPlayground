...
object MinimumTemperaturesByLocation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sc: SparkContext = new SparkContext("local[*]", "MinimumTemperaturesByLocation")
    val lines: RDD[String] = sc.textFile("data/1800.csv")
    val parsedLines: RDD[(String, String, Float)] = lines.map(parseLine)
    val minTemperatures: RDD[(String, String, Float)] = parsedLines.filter(x => x._2 == "TMIN")
    val stationTemperatures: RDD[(String, Float)] = minTemperatures.map(x => (x._1, x._3))
    val minimumTemperaturesByStation: RDD[(String, Float)] = stationTemperatures.reduceByKey((x, y) => min(x, y))
